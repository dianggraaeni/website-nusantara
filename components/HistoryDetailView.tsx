...
/**
 * 1. KOMPONEN HALAMAN DETAIL (FULL VIEW)
 */
const HistoryDetailView: React.FC<{ region: RegionHistory; onBack: () => void }> = ({ region, onBack }) => {
  const containerRef = useRef<HTMLDivElement>(null);

  useLayoutEffect(() => {
    // Reset scroll halaman ke paling atas setiap kali dibuka
    window.scrollTo(0, 0);
    if (containerRef.current) {
      containerRef.current.scrollTop = 0;
    }
    
    // Kunci scroll di body utama agar tidak goyang
  ...
  return (
    <div 
      ref={containerRef}
      className="fixed inset-0 z-[9999] bg-[#050505] overflow-y-auto font-sans text-white"
    ...
    >
      ...
      {/* HERO SECTION */}
      <div className="relative w-full h-[70vh] flex items-end overflow-hidden mt-[-100px]">
        <img 
          src={region.image} 
        ...
        <div className="relative z-10 px-6 md:px-20 w-full max-w-7xl mx-auto pb-16">
          ...
          <h1 className="text-white font-serif text-5xl md:text-8xl mb-6 leading-tight">
            {region.name}
          </h1>
        ...
        <div className="lg:col-span-4 h-fit lg:sticky lg:top-32 space-y-10">
          <div className="space-y-6">
            ...
            <p className="text-white/40 text-sm leading-relaxed font-light italic font-serif">
              Jejak sejarah luhur Nusantara yang abadi.
            </p>
          </div>
        </div>
              ...
              {region.timeline.map((item, i) => (
                <div key={i} className="relative pl-12 group">
                   ...
                   <div className="space-y-4">
                      <span className="text-gold font-serif text-6xl md:text-8xl font-bold block opacity-15">
                         {item.year}
                      </span>
                      <h4 className="text-white font-bold text-xl md:text-3xl uppercase tracking-widest">
                         {item.title}
                      </h4>
      ...
      <div className="py-40 text-center border-t border-white/5">
         <button onClick={onBack} className="px-12 py-5 border border-gold/40 text-gold text-[10px] font-bold uppercase tracking-[0.5em] hover:bg-gold hover:text-black transition-all rounded-full">
           Tutup Dokumentasi
         </button>
      </div>
    </div>
...
/**
 * 2. KOMPONEN UTAMA (HistorySection)
 */
const HistorySection: React.FC = () => {
  ...
  // LOGIKA PENTING: Jika ada region terpilih, tampilkan HANYA Detail View
  if (selectedRegion) {
    return <HistoryDetailView region={selectedRegion} onBack={() => setSelectedRegion(null)} />;
  }

  // Jika tidak ada region terpilih, tampilkan Carousel Utama
  return (
        ...
        <div className="relative w-full h-full flex items-center justify-center" style={{ transformStyle: 'preserve-3d' }}>
          {allProvinces.map((region, index) => {
            ...
            return (
              ...
              >
                <div className={`group relative aspect-[16/10] bg-[#0a0a0a] overflow-hidden rounded-xl border transition-all duration-700 ${style.isActive ? 'border-gold shadow-[0_0_80px_rgba(212,175,55,0.2)] cursor-pointer' : 'border-white/5 grayscale brightness-[0.2]'}`}>
